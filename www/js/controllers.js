angular.module('starter.controllers', ['firebase','ngCordova','ionic.service.core','ionic.service.push'])
  ...
  $scope.ref.authAnonymously(function(error, authData) {
      if (error) {
        ...
        $scope.liga.$loaded().then(function(data) {
            ...
            $cordovaSQLite.execute(db, query, [totales.Equipo, totales.GolDiferencia,totales.GolesContra,totales.GolesFavor,totales.LogoEquipo,totales.PartidosEmpatados,totales.PartidosGanados,totales.PartidosJugados,totales.PartidosPerdidos,totales.Puntos]).then(function(res) {
                 console.log("INSERT ID -> " + res.insertId);
            }, function (err) {
                console.log(err);
            });
...
.controller('ConfiguracionCtrl',['$scope', '$rootScope', '$window', '$localstorage' ,'$ionicPush','$ionicUser', function($scope, $rootScope, $window, $localstorage, $ionicPush,$ionicUser) {
    ...
    angular.extend(user, {
      name: 'quirquinchosanto',
      bio: 'verificacion para push notification'
    });
  ...
  $scope.setAlertNotifications = function() {
      ...
      if ($scope.data.alertNotificationsOn){
